...
def junk_line(line):
    ...
    is_copyright_info = bool(
        re.search(
            r'paper\s+title|technical\s+report|proceedings|preprint|to\s+appear|submission|(integrated|international).*conference|transactions\s+on|symposium\s+on|downloaded\s+from\s+http',
            line.lower()))
...
def extract_largest_text(obj, largest_text):
    ...
    for i, child in enumerate(obj):
        if isinstance(child, LTTextLine):
            ...
            largest_text = update_largest_text(obj.get_text(), child.y0,
                                               child.size, largest_text)
            # Only need to parse size of one char
            break
    return largest_text
    ...
    for child in lt_obj:
        ...
        if char_size != size:
            ...
            if state == CHAR_PARSING_STATE.INIT_X:
                ...
                if (char_distance > 0) and (char_current_distance <
                                            char_distance * 2.5):
            ...
            if (state == CHAR_PARSING_STATE.INSIDE_WORD) and (child.x1 <
                                                              char_previous_x1):
            ...
            # improve our heuristic
                    ...
                    char_current_distance > char_distance) and (
...
def valid_title(title):
    return not empty_str(title) and not junk_line(title) and empty_str(
