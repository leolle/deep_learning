#!/home/weiwu/.virtualenvs/nlp/bin/python
# -*- coding: utf-8 -*-
...
def get_url(title):
    ...
    return "%s/%s" % (options.urlbase, quote_plus(title))
...
placeholder_tag_patterns = [(re.compile(
    ...
    re.DOTALL | re.IGNORECASE), repl) for tag, repl in placeholder_tags.items()]
...
class TemplateArg(object):
    ...
    def subst(self, params, extractor, depth):
        ...
        if paramName in params:
            res = params[
                paramName]  # use parameter value specified in template invocation
...
#             out.write(header)
            ...
            while res is None:
                try:
                    ...
                    node.binaryContent = bytes(
                        HanziConv.toSimplified(joined_text), "utf-8")
    ...
    def extract(self, out):
        ...
        self.magicWords['NAMESPACENUMBER'] = options.knownNamespaces.get(
            ns, '0')
        ...
        errs = (self.template_title_errs, self.recursion_exceeded_1_errs,
                self.recursion_exceeded_2_errs, self.recursion_exceeded_3_errs)
        if any(errs):
            logging.warn(
    ...
    def transform(self, wikitext):
        ...
        for m in nowiki.finditer(wikitext, cur):
            res += self.transform1(
                wikitext[cur:m.start()]) + wikitext[m.start():m.end()]
            cur = m.end()
        # leftover
    ...
    def clean(self, text):
        ...
        for tag in options.discardElements:
            text = dropNested(text, r'<\s*%s\b[^>/]*>' % tag,
                              r'<\s*/\s*%s>' % tag)
        ...
        for pattern, placeholder in placeholder_tag_patterns:
            ...
            for match in pattern.finditer(text):
                text = text.replace(match.group(), '%s_%d' % (placeholder,
                                                              index))
                index += 1
    ...
    def expandTemplate(self, body):
        ...
        if colon > 1:
            funct = title[:colon]
            parts[0] = title[
                colon + 1:].strip()  # side-effect (parts[0] not used later)
        ...
        logging.debug('%*sTEMPLATE %s: %s', self.frame.depth, '', title,
                      template)
...
def string_replace(args):
    params = functionParams(args, ('source', 'pattern', 'replace', 'count',
                                   'plain'))
    source = params.get('source', '')
...
def roman_main(args):
    ...
    smallRomans = ((1000, "M"), (900, "CM"), (500, "D"), (400, "CD"),
...
parserFunctions = {
    ...
    '#ifexist':
    lambda extr, title, ifex, ifnex: extr.expand(ifnex),  # assuming title is not present
...
def callParserFunction(functionName, args, extractor):
    ...
    try:
        ...
        if functionName == '#invoke':
            ...
            logging.debug('%*s#invoke %s %s %s', extractor.frame.depth, '',
                          module, fun, args[2:])
            ...
            if len(args) == 2:
                ...
                params = [extractor.transform(p)
                          for p in args[2:]]  # evaluates them
...
def makeInternalLink(title, label):
    ...
    if colon == 0:
        ...
        if colon2 > 1 and title[colon + 1:
                                colon2] not in options.acceptedNamespaces:
            return ''
        ...
        # handle lists
            ...
            if line:  # FIXME: n is '"'
                if options.keepLists:
                    ...
                    page.append('{0:{1}s}'.format(bullet, len(listLevel)) +
                                line)
                elif options.toHTML:
...
def load_templates(file, output_file=None):
    ...
    for page_count, page_data in enumerate(pages_from(file)):
        ...
        if not output_file and (
                not options.templateNamespace or
    ...
    if output_file:
        output.close()
        logging.info("Saved %d templates to '%s'",
                     len(options.templates), output_file)
...
def process_dump(input_file, template_file, out_file, file_size, file_compress,
    ...
    if options.expand_templates:
        ...
        if template_file:
            if os.path.exists(template_file):
                logging.info("Loading template definitions from: %s",
                             template_file)
                # can't use with here:
        ...
        logging.info("Loaded %d templates in %.1fs",
                     len(options.templates), template_load_elapsed)
    ...
    reduce = Process(
        ...
        args=(options, output_queue, spool_length, out_file, file_size,
              file_compress))
    reduce.start()
...
def reduce_process(opts,
    ...
    while True:
        if next_page in spool:
            ...
            if len(spool) > 200:
                logging.debug('Collected %d, waiting: %d, %d',
                              len(spool), next_page, next_page == page_num)
...
def main():
    ...
    groupP.add_argument(
        "-l", "--links", action="store_true", help="preserve links")
    groupP.add_argument(
    ...
    groupP.add_argument(
        "--templates", help="use or create file containing templates")
    groupP.add_argument(
    ...
    groupP.add_argument(
        ...
        help=
        "Minimum expanded text length required to write document (default=%(default)s)"
    )
    groupP.add_argument(
        ...
        help=
        "Remove pages from output that contain disabmiguation markup (default=%(default)s)"
    )
    groupP.add_argument(
        "-it",
        "--ignored_tags",
    ...
    if args.ignored_tags:
        ...
        ignoredTags = [
            ...
            'h1', 'h2', 'h3', 'h4', 'hiero', 'i', 'kbd', 'p', 'plaintext', 's',
            'span', 'strike', 'strong', 'tt', 'u', 'var'
        ]
