...
for i in range(0,len(hero_data)):
	temp=Node('hero',name=str(hero_data['name'][i]),skill_passive=str(hero_data['skill_passive'][i]),skill_1=str(data['skill_1'][i]),skill_1_cooling=str(data['skill_1_cooling'][i]),skill_1_cost=str(data['skill_1_cost'][i]),skill_2=str(data['skill_2'][i]),skill_2_cost=str(data['skill_2_cost'][i]),skill_2_cooling=str(data['skill_2_cooling'][i]),R=str(data['R'][i]),R_cooling=str(data['R_cooling'][i]),R_cost=str(data['R_cost'][i]),tag=str(data['tag'][i]),HP=str(data['HP'][i]),MP=str(data['MP'][i]),HP_recover=str(data['HP_recover'][i]),MP_recover=str(data['MP_recover'][i]),attack=str(data['attack'][i]),defense=str(data['defense'][i]),magic_defense=str(data['magic_defense'][i]),speed=str(data['speed'][i]),attack_range=str(data['attack_range'][i]))
	# g.create(temp)
#建立关系，这里用的是Py2neo2
