...
class seleniumTest():
    ...
    def content(self, url):
        ...
        for iid in bsObj.find_all('div', class_='leftcolumn'):
            Statistics = iid.find_all(
                'div', class_='subheader')[0].text.replace('\n', '').replace(
                    '\t', '').strip(' ')
            Title = iid.find_all(
                'h1', class_={'title',
                              'mathjax'})[0].text.replace('\n', '').replace(
                                  '\t', '').strip(' ')
            Authors = iid.find_all(
                'div', class_='authors')[0].text.replace('\n', '').replace(
                    '\t', '').strip(' ')
            dateline = iid.find_all(
                'div', class_='dateline')[0].text.replace('\n', '').replace(
                    '\t', '').strip(' ')
            Abstract = iid.find_all(
                'blockquote', class_={'abstract', 'mathjax'})[0].text.replace(
                    '\n', '').replace('\t', '').strip(' ')
            Comments = iid.find_all(
                'td', class_={'tablecell', 'comments'})[1].text.replace(
                    '\n', '').replace('\t', '').strip(' ')
            # print dateline
            information = {
