...
from MagicGoogle_News.config import USER_AGENT, DOMAIN, BLACK_DOMAIN, URL_SEARCH, LOGGER
...
class MagicGoogle_News():
    ...
    def req_url(self, query, language=None,  start=0, pause=2):
        ...
        url = URL_SEARCH
        url = url.format(domain=domain,query=quote_plus(query),language=language,start=start)
        if language is None:
